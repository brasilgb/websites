...
const Posts = ({ posts, imagefeatured }: any) => {
  ...
  const coordinates = datasite?.config?.maps ? datasite?.config?.maps?.split(','): [-29.441221518021862,-51.510293465448264];
  const opening = datasite?.config?.opening ? datasite?.config?.opening.split(','): '';
  const center = useMemo(() => ({ lat: Number(coordinates[0]), lng: Number(coordinates[1]) }), []); //-29.441221518021862, -51.510293465448264
  const customMarker = {
              ...
              <div className="grid grid-cols-1 gap-12 mt-10 lg:grid-cols-2">
                <div className="grid grid-cols-1 gap-12 md:grid-cols-2 p-4 py-6 rounded-md shadow-md bg-gray-50 border-2 border-white md:p-8">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs text-gray-500">{opening[0]}</p>
                    <p className="mt-1 text-xs text-gray-500">{opening[1]}</p>
                    <p className="mt-1 text-xs text-gray-500">{opening[2]}</p>
                    <p className="mt-1 text-sm text-blue-500">{datasite?.config?.city} - {datasite?.config?.address}</p>
                  </div>
