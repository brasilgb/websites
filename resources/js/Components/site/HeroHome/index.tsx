        ...
        <div
          ...
          <div className="md:text-center lg:text-left">
            <h1 className="text-4xl tracking-tight font-extrabold text-gray-800 md:text-5xl">
              <span className="block xl:inline">{data[0]?.posts[0]?.title}</span>
            </h1>
            <p
              className="mt-3 text-base text-gray-500 md:mt-5 md:text-lg md:max-w-xl md:mx-auto lg:mx-0">
              {/* <div dangerouslySetInnerHTML={{ __html: data[0]?.posts[0]?.content }} /> */}
              {data[0]?.posts[0]?.summary}
            </p>

            <div className="mt-5 md:mt-8 md:flex md:justify-center lg:justify-start">
              <div className="rounded-md shadow">
                <Link href={route("slug", data[0]?.posts[0]?.slug)}
              ...
              <div className="mt-3 md:mt-0 md:ml-3">
                <a href={`https://api.whatsapp.com/send?phone=${data[0]?.posts[0]?.whatsapp}&text=Olá, poderia ajudar-me com uma dúvida?`}
          ...
          <div className="lg:inset-y-0 lg:right-0 lg:w-1/2 my-4">
            <img className="h-56 w-full object-cover md:h-96 lg:w-full lg:h-full" src={`/storage/uploads/${data[0]?.posts[0]?.featured}`} alt="" />
          </div>
